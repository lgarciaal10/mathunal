...
var SIM_NEW = {
 calcdif: [
  ...
  {id:`cd_c1_07`,c:1,t:`Límites algebraicos`,f:`alta`,e:`algebra`,tipo:`mcq`,dif:1,tEsp:60,
   texto:`Calcule $\\displaystyle\\lim_{x\\to3}\\dfrac{x^2-9}{x^2-x-6}$.`,
   opciones:[`$\\dfrac{6}{5}$`,`$1$`,`$0$`,`No existe`],correcta:0,
   resumen:`Forma $0/0$: se factoriza y se cancela $(x-3)$.`,
   pasos:[`$\\dfrac{(x-3)(x+3)}{(x-3)(x+2)} = \\dfrac{x+3}{x+2}$ para $x\\neq3$.`,
     `$\\displaystyle\\lim_{x\\to3}\\dfrac{x+3}{x+2} = \\dfrac{6}{5}$.`],
   ab:`$0/0$ en un cociente de polinomios → el factor $(x-a)$ está arriba y abajo. Factoriza y cancélalo.`,
   rc:`Sustituyes y da $0/0$ con polinomios → factoriza (Ruffini si hace falta).`},

  {id:`cd_c1_08`,c:1,t:`Límites al infinito`,f:`alta`,e:`conceptual`,tipo:`mcq`,dif:1,tEsp:55,
   texto:`Calcule $\\displaystyle\\lim_{x\\to\\infty}\\dfrac{3x^2-x+5}{2x^2+7}$.`,
   opciones:[`$\\dfrac{3}{2}$`,`$0$`,`$\\infty$`,`$1$`],correcta:0,
   resumen:`Mismo grado arriba y abajo → cociente de los coeficientes líderes.`,
   pasos:[`Divido todo por $x^2$: $\\dfrac{3-1/x+5/x^2}{2+7/x^2}$.`,
     `Cuando $x\\to\\infty$: $\\dfrac{3}{2}$.`],
   ab:`Racional en $\\infty$: grados iguales → coef. líder / coef. líder. Grado de arriba mayor → $\\pm\\infty$. Menor → $0$.`,
   rc:`"Límite cuando $x\\to\\infty$" de una fracción de polinomios → compara los grados.`},

  {id:`cd_c1_09`,c:1,t:`Límites notables`,f:`media`,e:`procedimiento`,tipo:`numerica`,dif:2,tEsp:60,
   texto:`Calcule $\\displaystyle\\lim_{x\\to0}\\dfrac{\\operatorname{sen}(5x)}{x}$.`,
   respuesta:`5`,unidad:``,tolNum:0.001,puntaje:15,
   resumen:`$\\dfrac{\\operatorname{sen}(5x)}{x} = 5\\cdot\\dfrac{\\operatorname{sen}(5x)}{5x} \\to 5\\cdot1$.`,
   pasos:[`Multiplico y divido por 5: $5\\cdot\\dfrac{\\operatorname{sen}(5x)}{5x}$.`,
     `$\\dfrac{\\operatorname{sen}(5x)}{5x}\\to1$, así que el límite es $5$.`],
   ab:`$\\dfrac{\\operatorname{sen}(ax)}{x}\\to a$. Fabrica el "$\\operatorname{sen} u/u$" metiendo el coeficiente.`,
   rc:`$\\operatorname{sen}$ de un múltiplo de $x$ sobre $x$ cuando $x\\to0$ → el coeficiente.`},

  {id:`cd_c1_10`,c:1,t:`Asíntotas`,f:`media`,e:`procedimiento`,tipo:`mcq`,dif:2,tEsp:80,
   texto:`La asíntota oblicua de $f(x)=\\dfrac{x^2+1}{x}$ cuando $x\\to\\pm\\infty$ es:`,
   opciones:[`$y = x$`,`$y = x+1$`,`$y = 0$`,`no tiene asíntota oblicua`],correcta:0,
   resumen:`Se divide: $\\dfrac{x^2+1}{x} = x + \\dfrac{1}{x}$.`,
   pasos:[`División: $\\dfrac{x^2+1}{x} = x + \\dfrac{1}{x}$.`,
     `Cuando $x\\to\\pm\\infty$, $\\dfrac{1}{x}\\to0$, así que $f(x)$ se pega a $y=x$.`],
   ab:`Asíntota oblicua: divide el numerador entre el denominador; el cociente (sin el resto) es la recta.`,
   rc:`Racional con grado de arriba = grado de abajo + 1 → hay asíntota oblicua. Haz la división.`},

  {id:`cd_c1_11`,c:1,t:`Límites indeterminados`,f:`baja`,e:`conceptual`,tipo:`mcq`,dif:2,tEsp:70,
   texto:`Calcule $\\displaystyle\\lim_{x\\to0} x^2\\operatorname{sen}\\!\\left(\\dfrac{1}{x}\\right)$.`,
   opciones:[`$0$`,`$1$`,`No existe`,`$\\infty$`],correcta:0,
   resumen:`Teorema del emparedado: $-x^2 \\le x^2\\operatorname{sen}(1/x) \\le x^2$.`,
   pasos:[`$-1 \\le \\operatorname{sen}(1/x) \\le 1$, así que $-x^2 \\le x^2\\operatorname{sen}(1/x) \\le x^2$.`,
     `$\\displaystyle\\lim_{x\\to0}(-x^2) = \\lim_{x\\to0} x^2 = 0$.`,
     `Por el emparedado, el límite es $0$.`],
   ab:`Algo acotado ($\\operatorname{sen}$, $\\cos$) por algo que tiende a 0 → el producto tiende a 0 (emparedado).`,
   rc:`Aparece $\\operatorname{sen}(1/x)$ o $\\cos(1/x)$ (oscila) multiplicado por algo que $\\to0$ → teorema del emparedado.`},
