...
\begin{minipage}[t]{0.62\linewidth}
  ...
  {\small 4 de Febrero del 2019 --- Versión A}\\
...
\[
\frac{dy}{dx}=\frac{x}{4y}+\frac{y}{x}, \qquad x>0,\ y>0.
\]
...
\item[(b)] \textbf{(15\%)} Un tanque semiesférico tiene un radio de 1 pie; el tanque está inicialmente lleno de agua y en el fondo tiene un orificio circular de $\frac{1}{12}$ pies de diámetro (ver figura). Plantee el PVI que describe la rapidez con que cambia la profundidad del agua $h(t)$ en el tanque en el instante $t$ a partir del instante inicial. (Tome la constante de proporcionalidad igual a 1)
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\scriptsize] at (-1.6,0.65) {1 pie};
\end{tikzpicture}
\end{center}
...
\begin{opciones}
\item[(a)] \textbf{(9\%)} Si $y=3+\dfrac{c}{x^3}$ es solución general de la ecuación diferencial lineal de primer orden
\[
xy'+3y=g(x),
\]
...
\]
\[
\text{O equivalentemente} \qquad \frac{dy}{dx}=\frac{x+y-1}{-x+y-4}.
\]
Con una sustitución adecuada, esta ecuación diferencial se transforma en una ecuación diferencial homogénea. Dicha ecuación diferencial homogénea en las variables $u$ y $v$ es
