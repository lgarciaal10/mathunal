...
\begin{minipage}[t]{0.62\linewidth}
  ...
  {\small 4 de Septiembre del 2017}\\
...
\]
\[
\left(x^4y^4-\frac{\arctan(x^2)}{1+\exp(\sin x)}\right)dx+\left(\frac{4}{5}x^5y^3-y^5\right)dy=0.
\]
...
\begin{opciones}
\item[(a)] \textbf{[15pt]} Suponga que el café en una taza tiene una temperatura de $210$°F cuando acaba de servirse, y que un minuto después se ha enfriado hasta tener una temperatura de $180$°F, todo ello en un recinto cuya temperatura es de $65$°F. Suponiendo que el fenómeno descrito obedece la ley de enfriamiento de Newton, escriba la ecuación diferencial y las condiciones adicionales que modelan la situación descrita.

\vspace{2.5cm}

\item[(b)] \textbf{[10pt]} Las curvas ortogonales a la familia $y+4x^3=k$, con $k\in\mathbb{R}$, satisfacen una ecuación diferencial. Escriba dicha ecuación diferencial, únicamente en las variables $x$ y $y$.
...
\[
\begin{cases}
\dfrac{dy}{dx}+\sqrt{y-x}=\ln(-x-2), \\[4pt]
y(x_0)=y_0.
\end{cases}
\tag{2}
\]
...
\begin{tikzpicture}[scale=0.42]
  ...
  \draw[-{Latex[length=2mm]}] (0,-4.5) -- (0,4.5) node[above] {$y$};
  \foreach \n in {-4,-3,-2,-1,1,2,3,4}{
  ...
  \node[font=\scriptsize] at (-2,4.8) {$x=-2$};
  % linea y=x
  \draw[dashed] (-4.5,-4.5) -- (4.5,4.5);
  \node[font=\scriptsize] at (4,4.6) {$y=x$};
  % region sombreada: x<-2 y y>=x
  \begin{scope}
    \clip (-4.5,-4.5) rectangle (-2,4.5);
    \clip plot[domain=-4.5:4.5] (\x,\x) -- (-4.5,4.5) -- cycle;
    \fill[pattern=north east lines, pattern color=black!70] (-4.5,-4.5) rectangle (-2,4.5);
  \end{scope}
\end{tikzpicture}
\end{center}
...
\item[(b)] \textbf{[5pt]} El atractor $y_1$ de la Ecuación (3) es\\
$y_1=$ \underline{\hspace{2cm}}
\item[(c)] \textbf{[5pt]} Si la función $y$ es solución de la Ecuación (3) y además satisface que $y(0)=-5$, entonces, la función es cóncava hacia\\
Respuesta: \underline{\hspace{2.5cm}}
\item[(d)] \textbf{[5pt]} Si la función $y$ es solución de la Ecuación (3) y satisface las condiciones $y(x_0)=y_0$, $\displaystyle\lim_{x\to\infty} y(x)=2$, entonces $y_0$ pertenece al intervalo $I=$ \underline{\hspace{2.5cm}}
\end{opciones}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \draw[dashed] (-3.3,2) -- (3.3,2) node[right,font=\scriptsize] {$y=2$};
  \draw[dashed] (-3.3,-2) -- (3.3,-2) node[right,font=\scriptsize] {$y=-2$};
  % familia de curvas sigmoides crecientes: de y=-2 (x->-inf) a y=2 (x->+inf)
  \foreach \k in {0.7,0.9,1.2,1.6,2.1}{
    \draw[thick] plot[domain=-3.2:3.2,samples=60] (\x,{-2+4/(1+exp(-1*min(max(\k*\x,-9),9)))});
  }
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.9]
  \draw[-{Latex[length=2mm]}] (-3.3,0) -- (3.3,0) node[right,font=\scriptsize] {$x$};
  \draw[-{Latex[length=2mm]}] (0,-1.8) -- (0,2.3) node[above,font=\scriptsize] {$y$};
  \draw[dashed] (-3.3,2) -- (3.3,2) node[right,font=\scriptsize] {$y=2$};
  \draw[dashed] (-3.3,-1.5) -- (3.3,-1.5) node[right,font=\scriptsize] {$y=-2$};
