...
\begin{minipage}[t]{0.62\linewidth}
  ...
  {\small 18 de Marzo de 2023 --- \textbf{Tema C}}\\
...
\begin{opciones}
\item[a)] \textbf{(4\%)} ¿Existe una solución no constante cuya gráfica atraviese la recta horizontal $p=6400$? Marque con una X la respuesta correcta

Sí\underline{\hspace{1cm}} No\underline{\hspace{1cm}}

\item[b)] \textbf{(6\%)} Si la función $p(t)$ es solución de la ecuación diferencial autónoma y además satisface que $p(0)=p_0=7450$, entonces $\displaystyle\lim_{t\to\infty}p(t)=$ \underline{\hspace{2cm}}

\item[c)] \textbf{(6\%)} Si la función $p(t)$ es solución de la ecuación diferencial autónoma, entonces el valor o los valores de $p_0= p(0)$ tales que $\displaystyle\lim_{t\to\infty}p(t)=6400$ es (son): \underline{\hspace{3cm}}
\end{opciones}

\columnbreak

\textbf{2. (25\%)} Un tanque con capacidad de 280 galones, contiene inicialmente 120 galones de agua, en las cuales hay disueltas 5 libras de sal. Suponga que $t$ es el tiempo (en minutos) a partir del instante inicial. Al tanque entra agua con una concentración de sal de $\dfrac{1}{t+120}$ libras por galón a razón de 2 galones por minuto y la mezcla resultante sale del tanque a razón de un galón por minuto.
...
\item[c)] \textbf{(10\%)} Un tanque semiesférico tiene un radio de 1 pie; el tanque está inicialmente lleno de agua y en el fondo tiene un orificio circular de $\frac{1}{24}$ pies de diámetro (ver figura). Plantee el PVI que describe la variación de la profundidad del agua $h(t)$ en el tanque en el instante $t$ a partir del instante inicial. (\textit{Tome la constante de proporcionalidad igual a 1})
...
\[
\begin{cases}
\dfrac{dy}{dx}=\dfrac{\sqrt{y}}{\sqrt{2-x}}, \\
y(x_0)=y_0.
\end{cases}
\tag{2}
\]
...
\[
y\,dx+(2xy-e^{-2y})\,dy=0, \qquad x>0,\ y>0.
\]
