...
\renewcommand{\examtitulo}{Cálculo en Varias Variables --- Segundo Parcial (Tema C)}
...
\[
\int_1^3 \int_0^{\ln y} f(x,y)\,dx\,dy
\]
...
\item{} (20\,\%) Considere el sólido en $\mathbb{R}^3$ encima del cono $z = \sqrt{x^2+y^2}$ debajo del plano $z = 3$. Y suponga que la densidad en cada punto $(x,y,z)$ es igual a la distancia del punto al eje $z$.
\begin{itemize}
...
\item{} (10\,\%) Plantee una integral en coordenadas esféricas que permita calcular la masa del sólido, en el primer octante, limitado por la esfera $x^2 + y^2 + z^2 = 16$, y cuya densidad en cada punto $(x,y,z)$ es proporcional al cuadrado de la distancia del punto al origen (no la evalúe).
...
\item{} (20\,\%) Determine el máximo y el mínimo de la función $f(x,y) = 4x^2 + y^2 - xy$ para los puntos $(x,y)$ que satisfacen $4x^2 + y^2 = 6$.

\item{} (20\,\%) Encuentre el momento respecto al eje $y$, $M_y$, de la región del primer cuadrante limitada por $y = x$, $y = \sqrt3\,x$ y $x^2 + y^2 = 16$, y cuya densidad en cada punto es $\rho(x,y) = \cos\!\left((x^2+y^2)^{3/2}\right)$.

\item{} (20\,\%) Sea $f(x,y) = (x - 1)(y^3 - y)$. Halle los puntos críticos de $f$ y clasifíquelos usando el criterio de las segundas derivadas.
\end{enumerate}
